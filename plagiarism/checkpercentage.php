...
// $sentences = preg_split('/(?<=[.?!])\s+(?=[a-z])/i', $getinput);

// echo($getinput);

$sentences=explode("akallakalkalklakal", $getinput);

// echo(sizeof($sentences));

foreach ($sentences as $mystr) {

  if(strlen($mystr)>3){
    if(searchonline($mystr)===1){
        $plag=$plag+1;
    }
    else{
      $uniq=$uniq+1;
...
// $str= explode(" ", $getinput);
// for ($i=0; $i < sizeof($str); $i+=12) {
//     $mystr="";
